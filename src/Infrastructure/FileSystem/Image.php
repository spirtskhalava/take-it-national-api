...
final class Image
{
     ...
     */
    public static function setImagine($imagine): void
    {
     ...
     */
    public static function crop($image, $width, $height, array $start = [0, 0]): ImageInterface
    {
     ...
     * @param bool $outbound whether to use outbound to resize
     * @param bool $forceSize whether to force the size we wish
     *
     * @throws \Imagine\Exception\InvalidArgumentException
     * @throws InvalidArgumentException
     * @throws RuntimeException
     ...
     * @return ImageInterface
     */
    public static function resizeInOut(
        $image,
        ?int $width,
        ?int $height,
        bool $outbound = false,
        ...
        if ($forceSize) {
            if ($size->getWidth() < $width) {
                $img->resize($size->widen($width));
            }
            if ($size->getHeight() < $height) {
                $img->resize($size->heighten($height));
            }
        }

        return $outbound
            ? $img->thumbnail(new Box($width, $height), ManipulatorInterface::THUMBNAIL_OUTBOUND)
            : $img->thumbnail(new Box($width, $height));
    }
    ...
    public static function text(
        ...
        $fontColor = $fontOptions['color'] ?? 'fff';
        $fontAngle = $fontOptions['angle'] ?? 0;
        $palette = new RGB();
     ...
     * @return ImageInterface
     */
    public static function frame($image, $margin = 20, $color = '666', $alpha = 100): ImageInterface
    {
        $img = self::ensureImageInterfaceInstance($image);
        $size = $img->getSize();
     ...
     */
    private static function createImagine(): ImagineInterface
    {
        ...
        throw new InvalidConfigException(
            'Your system does not support any of these drivers: ' . implode(',', (array)static::$driver)
        );
    }
     ...
     * @return ImageInterface
     */
    private static function ensureImageInterfaceInstance($image): ImageInterface
    {
     ...
     */
    private static function getThumbnailBox(BoxInterface $sourceBox, $width, $height): BoxInterface
    {
     ...
     */
    private static function getBox(BoxInterface $sourceBox, $width, $height, $keepAspectRatio = true): BoxInterface
    {
        if ($width === null && $height === null) {
     ...
     * @return bool
     */
    private static function isUpscaling(BoxInterface $sourceBox, BoxInterface $destinationBox): bool
    {
        return ($sourceBox->getWidth() <= $destinationBox->getWidth()
