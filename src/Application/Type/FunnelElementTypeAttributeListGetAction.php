...
final class FunnelElementTypeAttributeListGetAction extends AbstractAction
{
    private $funnelElementTypeTransformer;

    /**
     * FunnelElementTypeAttributeListGetAction constructor.
     * @param ContainerInterface $ci
     */
    public function __construct(ContainerInterface $ci)
    {
        $this->funnelElementTypeTransformer = $ci['funnel.element.type.attribute.transformer'];
    ...
    {
        try {
            $command = new GetFunnelElementTypeAttributeListCommand($request, (int) $args['id']);
            $data = $this->commandBus->handle($command);

            $data = $this->createCollection($data['items'], $request, $this->funnelElementTypeTransformer, null, $data['paginator']);
