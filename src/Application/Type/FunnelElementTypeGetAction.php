...
final class FunnelElementTypeGetAction extends AbstractAction
{
    /**
    ...
    /**
     * FunnelElementTypeGetAction constructor.
    ...
    {
        try {
            $command = new GetFunnelElementTypeCommand((int)$args['id'], $request);

            $data = $this->commandBus->handle($command);
            $data = $this->createItem($data, $this->funnelElementTypeTransformer);
