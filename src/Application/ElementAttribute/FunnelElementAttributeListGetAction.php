...
final class FunnelElementAttributeListGetAction extends AbstractAction
{
    private $funnelElementAttributeTransformer;

    /**
     * FunnelElementAttributeListGetAction constructor.
    ...
    {
        try {
            $command = new GetFunnelElementAttributeListCommand($request);
            $data = $this->commandBus->handle($command);

            $data = $this->createCollection(
                $data['items'],
                $request,
                $this->funnelElementAttributeTransformer,
                null,
                $data['paginator']
            );
