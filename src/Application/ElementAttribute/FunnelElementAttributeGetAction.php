...
final class FunnelElementAttributeGetAction extends AbstractAction
{
    /**
     * @var FunnelElementAttributeTransformer
     */
    private $funnelElementAttributeTransformer;
    /**
    ...
    {
        $this->funnelElementAttributeTransformer = $ci['funnel.element.attribute.transformer'];
    ...
    {
        try {
            $command = new GetFunnelElementAttributeCommand((int)$args['id']);

            $data = $this->commandBus->handle($command);
            $data = $this->createItem($data, $this->funnelElementAttributeTransformer);
