...
final class UserProfileGetAction extends AbstractAction
{
    /**
    ...
    /**
     * UserProfileGetAction constructor.
     * @param ContainerInterface $ci
     */
    public function __construct(ContainerInterface $ci)
    {
        $this->profileTransformer = $ci['profile.transformer'];
    ...
    {
        try {
            $command = new GetProfileCommand((int)$args['id']);

            $data = $this->commandBus->handle($command);
            $data = $this->createItem($data, $this->profileTransformer);
