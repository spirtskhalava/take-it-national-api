...
final class UserListGetAction extends AbstractAction
{
    /**
     * @var
     */
    ...
    /**
     * UserListGetAction constructor.
     * @param ContainerInterface $ci
     */
    public function __construct(ContainerInterface $ci)
    {
        $this->userTransformer = $ci['user.transformer'];
    ...
    {
        try {
            $command = new GetUserListCommand($request);
            $data = $this->commandBus->handle($command);

            $data = $this->createCollection($data['items'], $request, $this->userTransformer, null, $data['paginator']);
