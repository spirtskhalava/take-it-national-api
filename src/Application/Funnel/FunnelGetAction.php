...
final class FunnelGetAction extends AbstractAction
{
    /**
    ...
    {
        try {
            $command = new GetFunnelCommand((int)$args['id'], $request);

            $data = $this->commandBus->handle($command);
            $data = $this->createItem($data, $this->funnelTransformer);
